...
import 'package:bks/widgets/add_custom_book.dart';
                ...
                return SizedBox(
                  child: Column(
                    children: [
                      ...
                      SizedBox(
                        height: 60.h,
                        child: snapshot.data['items'] == null ? Center(child: Text('No books found', style: TextStyle(fontFamily: 'Hurme', fontSize: 10.sp),)) :ListView.builder(
                            shrinkWrap: true,
                            itemCount: snapshot.data['items'].length,
                            itemBuilder: (BuildContext context, int index){
                              return Column(
                                children: [
                                  snapshot.data['items'][index]['volumeInfo']['imageLinks'] == null ? Container(): InkWell(
                                    onTap: () async{
                                      PaletteGenerator paletteGenerator = await PaletteGenerator
                                          .fromImageProvider(
                                        Image.network(snapshot.data['items'][index]['volumeInfo']['imageLinks']['thumbnail']).image
                                      );
                                      ...
                                      if(table == 'current_books'){
                                        row = {
                                          ...
                                          DatabaseHelper.columnColor: paletteGenerator.dominantColor.color.toString(),
